...
def obtener_mapeo_columnas(hoja):
    """
    Crea un diccionario que mapea nombres de columnas a sus índices.
    Normaliza los nombres quitando espacios extras, saltos de línea y convirtiendo a mayúsculas.
    """
    mapeo = {}
    for cell in hoja[1]:  # Primera fila (encabezados)
        if cell.value:
            # Normalizar el nombre de la columna
            nombre_normalizado = str(cell.value).strip().upper().replace('\n', ' ')
            mapeo[nombre_normalizado] = cell.column - 1  # Índice basado en 0
    return mapeo

def obtener_valor_columna(fila, mapeo, nombre_columna):
    """
    Obtiene el valor de una columna por su nombre.
    Intenta varias variaciones del nombre para mayor flexibilidad.
    """
    nombre_normalizado = nombre_columna.strip().upper()
    
    # Lista de posibles variaciones del nombre
    variaciones = [
        nombre_normalizado,
        nombre_normalizado.replace('.', ''),
        nombre_normalizado.replace(' ', ''),
    ]
...
def cargaPermisos():
    ...
    for nombre_hoja in wb.sheetnames:
        ...
        print(f"\n--- Procesando hoja '{nombre_hoja}' para PERMISOS ---")
        print(f"Columnas encontradas: {list(mapeo.keys())}")

        for i, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True)):
            # Buscar valores por nombre de columna
            id_permiso = obtener_valor_columna(fila, mapeo, 'TIPO ACEX')
            des_permiso = obtener_valor_columna(fila, mapeo, 'AUTORIZACION')
            lucrativo = obtener_valor_columna(fila, mapeo, 'TASA 062')
            residencia = obtener_valor_columna(fila, mapeo, 'TASA 052')
            via_defecto = obtener_valor_columna(fila, mapeo, 'VIA ACEX')
            reglamento = obtener_valor_columna(fila, mapeo, 'NORMATIVA')
...
def cargaVias():
    ...
    for nombre_hoja in wb.sheetnames:
        ...
        print(f"\n--- Procesando hoja '{nombre_hoja}' para VIAS ---")
        print(f"Columnas encontradas: {list(mapeo.keys())}")

        for i, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True)):
            # Buscar valores por nombre de columna
            id_via = obtener_valor_columna(fila, mapeo, 'VIA ACEX')
            des_via_acceso = obtener_valor_columna(fila, mapeo, 'VIA')
...
def cargaAutorizaciones():
    ...
    for nombre_hoja in wb.sheetnames:
        ...
        print(f"\n--- Procesando hoja '{nombre_hoja}' para AUTORIZACIONES ---")
        print(f"Columnas encontradas: {list(mapeo.keys())}")

        for i, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True)):
            # Buscar valores por nombre de columna
            cod_MEYSS = obtener_valor_columna(fila, mapeo, 'COD.MEYSS')
            id_permiso = obtener_valor_columna(fila, mapeo, 'TIPO ACEX')
            id_via = obtener_valor_columna(fila, mapeo, 'VIA ACEX')
            plazo = obtener_valor_columna(fila, mapeo, 'PLAZO')
            silencio = obtener_valor_columna(fila, mapeo, 'SILENCIO')
            tasa_052 = obtener_valor_columna(fila, mapeo, 'TASA 052')
            tasa_062 = obtener_valor_columna(fila, mapeo, 'TASA 062')
            tasa_dos_veces_smi = obtener_valor_columna(fila, mapeo, 'TASA 062 2 VECES SMI')
            autoriza_trabajar = obtener_valor_columna(fila, mapeo, 'AUTORIZA A  TRABAJAR')
            duracion = obtener_valor_columna(fila, mapeo, 'DURACIÓN INICIAL MÁXIMA (SI NO HAY TRABAJO ES LA DURACIÓN DE LA ESTANCIA O RESIDENCIA)')
