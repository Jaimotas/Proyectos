...
def obtener_mapeo_columnas(hoja):
    """
    Crea un diccionario que mapea nombres de columnas NORMALIZADOS a sus índices.
    """
    mapeo = {}
    for cell in hoja[1]:  # Primera fila (encabezados)
        if cell.value:
            # Normalizar completamente: quitar puntos, espacios múltiples, etc
            nombre_original = str(cell.value).strip().upper()
            
            # Normalización completa para búsqueda
            nombre_normalizado = nombre_original.replace('\n', ' ')
            nombre_normalizado = ' '.join(nombre_normalizado.split())  # Quitar espacios múltiples
            nombre_normalizado = nombre_normalizado.replace('.', '')  # Quitar puntos
            nombre_normalizado = nombre_normalizado.replace('  ', ' ')  # Espacios dobles
            
            mapeo[nombre_normalizado] = cell.column - 1  # Índice basado en 0
            
    return mapeo

def obtener_valor_columna(fila, mapeo, *nombres_posibles):
    """
    Obtiene el valor de una columna buscando entre varios nombres posibles.
    Normaliza los nombres para hacer coincidir con el mapeo.
    
    Ejemplo: obtener_valor_columna(fila, mapeo, 'TIPO ACEX', 'CODIGO ACEX', 'COD ACEX')
    """
    for nombre_columna in nombres_posibles:
        # Normalizar el nombre buscado igual que los encabezados
        nombre_normalizado = nombre_columna.strip().upper()
        nombre_normalizado = ' '.join(nombre_normalizado.split())
        nombre_normalizado = nombre_normalizado.replace('.', '')
        nombre_normalizado = nombre_normalizado.replace('  ', ' ')
        
        # Buscar en el mapeo
        if nombre_normalizado in mapeo:
            indice = mapeo[nombre_normalizado]
            if indice < len(fila):
                return fila[indice]
...
def cargaPermisos():
    ...
    # Recolectar todos los permisos únicos de todas las hojas primero
    permisos_dict = {}
    
    for nombre_hoja in wb.sheetnames:
        hoja = wb[nombre_hoja]
        mapeo = obtener_mapeo_columnas(hoja)

        for i, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True)):
            # Buscar valores por nombre de columna (con variantes)
            id_permiso = obtener_valor_columna(fila, mapeo, 'TIPO ACEX', 'CODIGO ACEX', 'COD ACEX', 'PERMISO', 'CODIGO ACEX?')
            des_permiso = obtener_valor_columna(fila, mapeo, 'AUTORIZACION', 'AUTORIZACIÓN')
            lucrativo = obtener_valor_columna(fila, mapeo, 'TASA 062', 'TASA 062 2 VECES SMI')
            residencia = obtener_valor_columna(fila, mapeo, 'TASA 052')
            via_defecto = obtener_valor_columna(fila, mapeo, 'VIA ACEX', 'VIA')
            reglamento = obtener_valor_columna(fila, mapeo, 'NORMATIVA', 'REAL DECRETO')

            # Saltar filas incompletas - permitir reglamento vacío
            if not id_permiso or not des_permiso or not via_defecto:
                continue
            
            # Si reglamento es None, usar valor por defecto
            if not reglamento:
                reglamento = 'N/A'
            
            # Solo guardar el primero que encontremos de cada ID
            if id_permiso not in permisos_dict:
                def normalizar_valor(valor):
                    return 'N' if valor in ("N/A", None) else 'S'
                
                permisos_dict[id_permiso] = {
                    'des_permiso': des_permiso,
                    'lucrativo': normalizar_valor(lucrativo),
                    'residencia': normalizar_valor(residencia),
                    'via_defecto': via_defecto,
                    'reglamento': reglamento
                }
    
    # Ahora insertar todos los permisos únicos
    print(f"\n--- Insertando {len(permisos_dict)} permisos únicos ---")
    for id_permiso, datos in permisos_dict.items():
        try:
            cursor.execute(
                "INSERT INTO LGA_PERMISOS (ID, DES_PERMISO, LUCRATIVO, RESIDENCIA, VIA_DEFECTO, REGLAMENTO) VALUES (%s, %s, %s, %s, %s, %s)",
                (id_permiso, datos['des_permiso'], datos['lucrativo'], datos['residencia'], datos['via_defecto'], datos['reglamento'])
            )
        except IntegrityError as e:
            print(f"Error insertando {id_permiso}: {e}")

    conexion.commit()
    print(f"Total de permisos procesados: {len(permisos_dict)}")

def cargaVias():
    cursor.execute("DELETE FROM lga_via_acceso")
    
    # Recolectar todas las vías únicas de todas las hojas primero
    vias_dict = {}
    ...
    for nombre_hoja in wb.sheetnames:
        ...
        for i, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True)):
            ...
            id_via = obtener_valor_columna(fila, mapeo, 'VIA ACEX', 'VIA')
            des_via_acceso = obtener_valor_columna(fila, mapeo, 'VIA', 'VÍA', 'VIA ACCESO/SUPUESTO')
            ...
            # Solo guardar el primero que encontremos de cada ID
            if id_via not in vias_dict:
                vias_dict[id_via] = des_via_acceso
    
    # Ahora insertar todas las vías únicas
    print(f"\n--- Insertando {len(vias_dict)} vías únicas ---")
    for id_via, des_via_acceso in vias_dict.items():
        try:
            cursor.execute(
            ...
            print(f"Error insertando {id_via}: {e}")

    conexion.commit()
    print(f"Total de vías procesadas: {len(vias_dict)}")
...
def cargaAutorizaciones():
    ...
    for nombre_hoja in wb.sheetnames:
        ...
        for i, fila in enumerate(hoja.iter_rows(min_row=2, values_only=True)):
            # Buscar valores por nombre de columna (con variantes)
            cod_MEYSS = obtener_valor_columna(fila, mapeo, 'COD.MEYSS', 'COD MEYSS', 'CODMEYSS', 'MEYSS', ' MEYSS')
            id_permiso = obtener_valor_columna(fila, mapeo, 'TIPO ACEX', 'CODIGO ACEX', 'COD ACEX', 'PERMISO', 'CODIGO ACEX?')
            id_via = obtener_valor_columna(fila, mapeo, 'VIA ACEX', 'VIA')
            plazo = obtener_valor_columna(fila, mapeo, 'PLAZO')
            silencio = obtener_valor_columna(fila, mapeo, 'SILENCIO', 'SENTIDO SILENCIO')
            tasa_052 = obtener_valor_columna(fila, mapeo, 'TASA 052')
            tasa_062 = obtener_valor_columna(fila, mapeo, 'TASA 062')
            tasa_dos_veces_smi = obtener_valor_columna(fila, mapeo, 'TASA 062 2 VECES SMI', 'TASA 062                          2 VECES SMI')
            autoriza_trabajar = obtener_valor_columna(fila, mapeo, 'AUTORIZA A  TRABAJAR', 'AUTORIZA A TRABAJAR', 'AUTORIZA A \nTRABAJAR')
            duracion = obtener_valor_columna(fila, mapeo, 'DURACIÓN INICIAL MÁXIMA (SI NO HAY TRABAJO ES LA DURACIÓN DE LA ESTANCIA O RESIDENCIA)', 'DURACION INICIAL MAXIMA', 'DURACIÓN', 'DURACIÓN\n')
            ...
            try:
                ...
                # Extraer solo caracteres numéricos del plazo
                num_plazos_str = ''.join(filter(str.isdigit, str(plazo)))
                num_plazos = int(num_plazos_str) if num_plazos_str else 0
                
                # Formatear tipo_plazo
                tipo_plazo = formateo_tipo_plazo(str(plazo))
